...
test.describe('OffGrid1 Website Testing', () => {
  ...
  test('Solar Load Analysis - Auto-fill Small Family preset works', async ({ page }) => {
    ...
    const applianceItems = page.locator('[class*="appliance"]').or(page.locator('li')).filter({ hasText: /W|watts/i });
  ...
  test('Rainwater Calculator - Auto-fill Small Family preset works', async ({ page }) => {
    ...
    const roofMaterialSelect = page.locator('select').filter({ has: page.locator('option[value="asphalt"]') });
  ...
  test('Battery Sizing Calculator - Auto-fill preset works', async ({ page }) => {
    ...
    const backupDaysInput = page.locator('input[placeholder*="days" i], input[placeholder*="backup" i]').first();
  ...
  test('Wind Power Calculator - Preset and calculation', async ({ page }) => {
    ...
    const presetBtn = page.locator('button').filter({ hasText: /Family|Home|Off-Grid/ }).first();
